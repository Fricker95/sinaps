...
from scipy.sparse import dia_matrix, csr_matrix, spdiags, csr_array, lil_matrix
...
class Neuron(param.Parameterized):
	...
	def _difus_end(self, ion):
		...
		return (
			np.ravel(np.array([s._difus_end(ion) for s in self.sections])
			.swapaxes(0, 1))
		)
	...
	@lru_cache(128)
	def _get_difus(self, ion, m):
		difus = self._difus_array(ion)
		d = np.concatenate([difus, difus[:m]])
...
class _SimuChannel:
	...
	def __init__(self, ch_cls):
		...
		if hasattr(ch_cls, "_J"):
			self.J = ch_cls._J
		if hasattr(ch_cls, "_dI"):
			self.dI = ch_cls._dI
		if hasattr(ch_cls, "_ddS"):
			self.ddS = ch_cls._ddS
		self.type = ch_cls
	...
	@profile
	def fill_dI_ddS(self, y, J, t):
		n = len(self.idS) 
		V = y[self.idV, :]
		S = [y[self.idS[i], :] for i in range(n)]

		if not hasattr(self, 'position'): 
			idX = [self.idV, *self.idS]
			n += 1 

			if hasattr(self, 'dI'):
				dI = self.dI(V, *S, t, **self.params)
			for i in range(n):
				J[self.idV, idX[i]] += np.squeeze(next(dI) * self.k)

				if hasattr(self, 'ddS'):
					ddS = self.ddS(V, *S, t, **self.params)
				for i in range(1, n):
					for j in range(n):
